...
class Signature {
   ...
   */
  verify(signature) {
    if (!Signature.constantTimeSafeEquals(Buffer.from(signature), Buffer.from(this.signature))) {
  ...
  /**
   * A constant-time comparison function so we don't leak
   * information about the contents of either string.
   *
   ...
   * @return {boolean}
   */
  static constantTimeSafeEquals(a, b) {
