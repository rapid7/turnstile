...
class TurnstileClient {
  ...
  constructor(conf, request) {
    checkArgument(conf.digest, 'conf.digest is a required argument');
    checkArgument(conf.payload, 'conf.payload is a required argument');
    checkArgument(conf.secret, 'conf.secret is a required argument');
    ...
    case 'hmac':
      ...
      break;
    case 'jwt':
      const token = jwt.sign(this._payload, this._secret);
